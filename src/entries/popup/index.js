import { getHostname } from '../../shared/utils/url.js';
...
async function init() {
  ...
  const blockedUrls = await dataManager.getBlockedUrls();
  ...
  quickBlockBtn.addEventListener("click", async () => {
    try {
      const latest = await dataManager.getBlockedUrls();
      ...
      next.push(hostname);
      await dataManager.setBlockedUrls(next);
